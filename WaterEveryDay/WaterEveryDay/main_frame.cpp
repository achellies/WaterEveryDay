...
void MainFrame::Notify(TNotifyUI& msg)
{
	...
	else if (_tcsicmp(msg.sType, kClick) == 0)
	{
		...
		else if (_tcsicmp(msg.pSender->GetName(), _T("buybtn")) == 0)
		{
			if (thread_ != NULL)
			{
				if (DUIMessageBox(m_hWnd, _T("确定要停止领水吗？"), _T("警告"), MB_OKCANCEL) == IDOK)
				{
					CAutoEvent autoLock(lock_);
					thread_cancel_ = true;
				}
			...
			if (apkFile.empty())
			{
				DUIMessageBox(m_hWnd, _T("请选择要进行友宝客户端的的APK文件!"), _T("警告"));
				return;
			}
			...
			{
				DUIMessageBox(m_hWnd, _T("请到设置面板中进行相关设置后在进行领水!"), _T("警告"));
				return;
			}
			...
			msg.pSender->SetText(_T("停止领水"));

			DWORD dwThreadId = 0;
			HANDLE hEvent = CreateEvent(NULL, TRUE, FALSE, _T("xxxx"));

			thread_ = CreateThread(NULL, 0, BuyWaterThread, (LPVOID)this, 0, &dwThreadId);
...
DWORD MainFrame::BuyWaterThread(LPVOID lpVoid)
{
	...
	int code = 0;
	{
		tString myUboxApkName = _T("ubox_achellies-unsigned.apk");
		tString uboxApkFileFolder = apkFile.substr(0, apkFile.find_last_of(_T("\\")));
		tString uboxApkFileName = apkFile.substr(apkFile.find_last_of(_T("\\")) + 1);
		uboxApkFileName = uboxApkFileName.substr(0, uboxApkFileName.find_last_of(_T(".")));

		pThis->DeleteDirectory(tString(uboxApkFileFolder + _T("\\") + uboxApkFileName).c_str());
		DeleteFile(tString(uboxApkFileFolder + _T("\\") + myUboxApkName).c_str());

		// 使用apktool反编译友宝客户端
		tString log;
		_stprintf(szBuf, _T("使用apktool反编译友宝客户端"));
		log = szBuf;
		pThis->AddLogMessage(log);
		...
		tString current_folder = pThis->paint_manager_.GetInstancePath().GetData();		
		_stprintf(szBuf, _T("cd \"%s\" & cd %s"), current_folder.c_str(), current_folder.substr(0, current_folder.find_first_of(_T(":")) + 1).c_str());
		int code = system(StringConvertor::TcharToAnsi(szBuf));

		tString apktool = pThis->paint_manager_.GetInstancePath() + _T("apktool");
		_stprintf(szBuf, _T("apktool d %s %s"), apkFile.c_str(), pause ? _T("& pause") : _T(""));
		code = system(StringConvertor::TcharToAnsi(szBuf));

		// 产生随机的设备ID和MAC地址		
		_stprintf(szBuf, _T("产生随机的设备ID和MAC地址"));
		log = szBuf;
		pThis->AddLogMessage(log);

		char szHexBuf[MAX_PATH] = {0};
		//     const-string v0, "458342047634282"
		string deviceId;
		srand(GetTickCount());
		deviceId = "\"";
		for (int i = 0; i < 15; i++)
		{
			DWORD word = rand();
			sprintf(szHexBuf, "%d", word % 10);
			deviceId += szHexBuf;
		}
		deviceId += "\"";

		//     const-string v1, "6C:83:36:0C:C5:88"
		string macAddress;
		...
		*/
		...
		// 更改smail代码
		_stprintf(szBuf, _T("更改smail代码"));
		log = szBuf;
		pThis->AddLogMessage(log);

		// 使用apktool重新进行打包
		_stprintf(szBuf, _T("使用apktool重新进行打包"));
		log = szBuf;
		pThis->AddLogMessage(log);

		tString smailCodeFile = tString(uboxApkFileFolder + _T("\\") + uboxApkFileName + _T("\\smali\\com\\ub\\main\\util\\Tool.smali"));
		ifstream input;
		input.open(smailCodeFile.c_str());

		std::vector<string> smailCode;
		if ( input.is_open() && input.peek() != EOF)
		{
			char szCodeBuf[MAX_PATH * 4] = {0};
			std::string code;
			input.seekg(0);
			memset(szCodeBuf, 0x00, MAX_PATH * 4);
			input.getline(szCodeBuf, MAX_PATH * 4);
			code = szCodeBuf;
			while(!input.eof() || !code.empty())
			{
				smailCode.push_back(code);
				code.clear();
				memset(szCodeBuf, 0x00, MAX_PATH * 4);
				input.getline(szCodeBuf, MAX_PATH * 4);
				code = szCodeBuf;
		...
		// 修改deviceID
		bool getDeviceIdFound = false;
		bool getMacAddressFound = false;
		for (vector<string>::iterator iter = smailCode.begin(); iter != smailCode.end(); ++iter)
		{
			if (strstr(iter->c_str(), ".method public static getDeviceId(Landroid/content/Context;)Ljava/lang/String;") != NULL)
			{
				getDeviceIdFound = true;
			}
			else if (getDeviceIdFound && strstr(iter->c_str(), "    const-string v0, \"\""))
			{
				getDeviceIdFound = false;
				*iter = "    const-string v0, " + deviceId;
				vector<string>::iterator iter2 = iter;
				for (;iter2 != smailCode.end(); ++iter2)
				{
					if (strstr(iter2->c_str(), "    return-object v0"))
					{
						break;
					}
				}
				smailCode.erase(iter + 1, iter2);
				break;
			}
		}
		...
		for (vector<string>::iterator iter = smailCode.begin(); iter != smailCode.end(); ++iter)
		{
			if (strstr(iter->c_str(), ".method public static getMacAddress(Landroid/content/Context;)Ljava/lang/String;") != NULL)
			{
				getMacAddressFound = true;
			}
			else if (getMacAddressFound && strstr(iter->c_str(), "    const-string v1, \"\""))
			{
				getMacAddressFound = false;
				*iter = "    const-string v1, " + macAddress;
				vector<string>::iterator iter2 = iter;
				for (;iter2 != smailCode.end(); ++iter2)
				{
					if (strstr(iter2->c_str(), "    return-object v1"))
					{
						break;
					}
				}
		...
		ofstream output;
		output.open(smailCodeFile.c_str(), ios_base::out | ios_base::trunc | ios_base::binary);
		...
		_stprintf(szBuf, _T("cd \"%s\" & cd %s"), current_folder.c_str(), current_folder.substr(0, current_folder.find_first_of(_T(":")) + 1).c_str());
		code = system(StringConvertor::TcharToAnsi(szBuf));
		_stprintf(szBuf, _T("apktool b %s %s %s"), uboxApkFileName.c_str(), myUboxApkName.c_str(), pause ? _T("& pause") : _T(""));
		code = system(StringConvertor::TcharToAnsi(szBuf));

		// 进行签名
		pThis->SignApk(uboxApkFileFolder + _T("\\") + myUboxApkName, pThis->outputPath_);
	}
...

	{
		...
		CControlUI* pButton = pThis->paint_manager_.FindControl(_T("buybtn"));
		if (pButton != NULL)
			pButton->SetText(_T("领水"));
	}
...
void MainFrame::SignApk(const tString& old_apk, const tString& output_path)
{
	...
	tString apkSigned = output_path + _T("ubox_achellies-signed.apk");
